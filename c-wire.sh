...
duree_fichier=$(echo "$(date +%s.%N) - $debut" | bc)
echo -e "Fichier '\033[1m$fichier_sortie\033[0m' généré en \033[1m$(printf "%.2f" $duree_fichier) secondes\033[0m"
...
if [[ "$type_station" == "lv" && "$type_consommateur" == "all" && -z "$identifiant_centrale" ]]; then
    ...
    duree_minmax=$(echo "$(date +%s.%N) - $debut - $duree_fichier" | bc)
    echo -e "Fichier '\033[1m$minmax_sortie\033[0m' généré en \033[1m$(printf "%.2f" $duree_minmax) secondes\033[0m"
    ...
    duree_graph=$(echo "$(date +%s.%N) - $debut - $duree_fichier - $duree_minmax" | bc)
    echo -e "Graphique '\033[1mlv_all_minmax.png\033[0m' généré en \033[1m$(printf "%.2f" $duree_graph) secondes\033[0m" 
fi
...
echo -e "\nDurée de traitement totale : \033[1m$(printf "%.2f" $duree) secondes\033[0m\n"
